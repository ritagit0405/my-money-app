...
if not df.empty:
    ...
    c_m2.metric("💸 當月總支出", f"{m_expense:,.0f} 元")
    c_m3.metric("⚖️ 本月結餘", f"{(m_income - m_expense):,.0f} 元")

    st.subheader(f"🗓️ {history_month[:4]} 年度累計統計")
    c_y1, c_y2, c_y3 = st.columns(3)
    c_y1.metric("📈 當年度總收入", f"{y_income:,.0f} 元")
    c_y2.metric("📉 當年度總支出", f"{y_expense:,.0f} 元")
    c_y3.metric("🏛️ 當年度總結餘", f"{(y_income - y_expense):,.0f} 元")
